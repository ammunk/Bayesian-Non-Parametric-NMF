...
class BNPWFactor(FactorTruncNormVB):
    ...
    def update(self, X, mean_lambda, mean_tau, mean_R, mean_sq_sum_R, mean_Z,
        ...
        self.sigma_sq = np.reciprocal(mean_tau*mean_sq_sum_R*mean_sq_Z
                                      + mean_lambda[:,d])
    ...
    def elbo_part(self, mean_lambda, ln_mean_lambda):
        ...
        prior_elbo = np.sum(-np.log(2*np.pi) + ln_mean_lambda
                            - self.mean_sq*mean_lambda)
        entropy_elbo = np.sum(self.etrpy)
...
class FactorBernoulli:
    ...
    def initialize(self, est_mean, est_var):
        ...
        hyperparameters
        """
        self.mean = est_mean + self.eps
        self.var = est_var + self.eps
        self.mean_sq = mean_sq(self.mean, self.var)
        self.cal_mean_sq_sum()

    def update(self, X, mean_ln_pi, mean_one_ln_pi, mean_tau, mean_W,
               mean_sq_W, mean_R, mean_sq_sum_R):
        ...
        for d in np.random.permutation(self.D):
            prod_mix = WRX[:,d] - mean_W[:,d] * ((mean_W*self.mean)
                                         @ RR_zero_diag[:,d])
            self.pi[:,d] =  mean_tau*(prod_mix
                                      - 0.5*mean_sq_W[:,d]*mean_sq_sum_R[d])
...
class FactorPriorBNP(FactorPriorGamma):

    def __init__(self, lambda_alpha, lambda_beta):
        super().__init__(lambda_alpha, lambda_beta)
        self.alpha = lambda_alpha + 0.5

    def update(self, factor_mean_sq):
        self.beta = self.lambda_beta + 0.5 * factor_mean_sq
...
class BetaPrior:
    ...
    def elbo_part(self):
        ln_B = -(gammaln(self.alpha) + gammaln(self.beta) \
               - gammaln(self.alpha + self.beta))
        prior_elbo = np.sum((self.alpha - 1)*self.ln_mean
                            + (self.beta - 1)*self.ln_o_mean + ln_B)
        entropy_elbo = np.sum(self.etrpy)
